...
Зависимости проверяются для Python 3.11–3.14.
...
- 3D-модели имеют встроенный структурный просмотр даже без `trimesh`;
...
В реестре программы сейчас 290 уникальных расширений. Это число включает как полноценную
отрисовку, так и структурный или безопасный метаданный предпросмотр сложных форматов.
Полный список показывается в приложении через **Вид → Поддерживаемые форматы** и приведён
в [`docs/FORMATS.md`](docs/FORMATS.md).

## Документация

- [`docs/USER_GUIDE.md`](docs/USER_GUIDE.md) — установка, работа с интерфейсом,
  команды сборки и решение типовых проблем;
- [`docs/FORMATS.md`](docs/FORMATS.md) — точный список расширений и глубина поддержки;
- [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) — устройство реестра и правила
...
- Реальная воспроизводимость аудио и видео зависит от декодеров, включённых в
  Qt Multimedia/FFmpeg для конкретной платформы.
